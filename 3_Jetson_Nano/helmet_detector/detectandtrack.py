...
def YOLO(videopath):
    ...
    if netMain is None:
        netMain = helmet_darknet.load_net_custom(configPath.encode(
            "ascii"), weightPath.encode("ascii"), 0, 1)  # batch size = 1
    if metaMain is None:
        metaMain = helmet_darknet.load_meta(metaPath.encode("ascii"))
    if altNames is None:
        try:
    ...
    darknet_image = helmet_darknet.make_image(helmet_darknet.network_width(netMain),
                                    helmet_darknet.network_height(netMain),3)
    # network image size (416*416, ...)
    network_image_size = (helmet_darknet.network_width(netMain),
                          helmet_darknet.network_height(netMain))
    fps = 0.0
    ...
    while True:
        ...
        if ret:
            ...
            helmet_darknet.copy_image_from_bytes(darknet_image, frame_resized.tobytes())

            # get inference information from Yolov4 Model (class, probability, (x,y,width,height))
            detections = helmet_darknet.detect_image(netMain, metaMain, darknet_image, thresh=0.25)
